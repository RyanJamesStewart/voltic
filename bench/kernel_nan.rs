...
fn main() {
    ...
    let raw = implied_vol_fast_kernel(&ds.spot, &ds.strike, &ds.tte, &ds.rate, &ds.price, &ds.kind);
    ...
    println!("n = {n}");
    println!(
        "kernel NaN overall: {tot_nan} ({:.4}%)",
        100.0 * tot_nan as f64 / n as f64
    );
    println!(
        "kernel max |err| on accepted lanes: {:.3e}",
        max_err_accepted
    );
    println!("per-band kernel NaN%:");
    println!(
        "  deep OTM (<0.7)  : {otm_nan:>7}/{otm_n:<7} = {:.4}%   max|err| accepted {:.3e}",
        100.0 * otm_nan as f64 / otm_n.max(1) as f64,
        otm_max_err
    );
    println!(
        "  near ATM (0.95-1.05): {atm_nan:>7}/{atm_n:<7} = {:.4}%   max|err| accepted {:.3e}",
        100.0 * atm_nan as f64 / atm_n.max(1) as f64,
        atm_max_err
    );
    println!(
        "  deep ITM (>1.3)  : {itm_nan:>7}/{itm_n:<7} = {:.4}%   max|err| accepted {:.3e}",
        100.0 * itm_nan as f64 / itm_n.max(1) as f64,
        itm_max_err
    );
    println!(
        "  other            : {other_nan:>7}/{other_n:<7} = {:.4}%",
